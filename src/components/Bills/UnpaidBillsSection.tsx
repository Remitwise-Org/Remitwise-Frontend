...
import { Bill, mockBills } from '@/lib/mockdata/bills';
import { BillUrgency } from '@/lib/bills/urgency';
import { WidgetEmptyState } from '@/components/ui/WidgetStates';

export function UnpaidBillsSection() {
    const { density } = useDensity();
    const unpaidStatuses: BillUrgency[] = ['overdue', 'urgent', 'upcoming'];
    ...
    const billsByStatus = unpaidBills.reduce((acc: Record<BillUrgency, Bill[]>, bill: Bill) => {
        (acc[bill.status] = acc[bill.status] || []).push(bill);
        return acc;
    }, {} as Record<BillUrgency, Bill[]>);

    const statusOrder: BillUrgency[] = ['overdue', 'urgent', 'upcoming'];
